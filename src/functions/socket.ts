...
const handleSuggestion = (request: IRequest) => {
  ...
  const suggestion: ISendMessage = {
    author: AUTHOR_ENUM.CHATBOT,
    message: lastWord
      ? possibleAnswers
          .map((i) => {
...
const handleChat = (request: IRequest) => {
  ...
  const { missedSize, nameRequested } = data;
  const message = _message.toLowerCase();
  const response: IResponse & ISendMessage = {
    message: "",
    author: AUTHOR_ENUM.CHATBOT,
    date: new Date().toString(),
  };
  ...
  if (init) {
    ...
    if (!response.message && !response.list?.length) {
      response.message =
        "Excuse me. I did not understand what you want. Could you try again?";
    }
  }
