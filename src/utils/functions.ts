...
import { coffees, connections, counts, hellos, lists, NODE_ENV, sizes, standardAnswers } from "./constants";
import { CoffeeCountType, CoffeeSizeType, CoffeeType, ICoffeeOrder, IConnection, IConnectionData, IRequest, IResponse } from "./types";
...
export const createConnection: (request: request) => [IConnection, string] = (request: any) => {
    const id = guid();
...
export const sendMessage = (message: any, id: string) => {
...
export const getConnection: (id: string) => IConnection = (id) => connections[id];
...
export const fixMissedSize = (size: CoffeeSizeType, data: IConnectionData, response: IResponse, request: IRequest) => {
    const { oldRequestedCoffees } = data;
    const isValidSize = sizes.map(i => i.name).filter(i => i === size).length > 0;
...
export const prepareStandardAnswers = (request: IRequest, response: IResponse) => {
    let standardAnswer = "";
...
export const findRequest = (request: IRequest, response: IResponse) => {
    ...
    const { oldCount, missedSize, name, nameRequested, oldCoffee, oldRequestedCoffees, oldSize } = data;
    ...
    for (let i = 0; i < coffeeRegex.length; i++) {
        ...
        if (match) {
            ...
            let count = oldCount || "", size: CoffeeSizeType | string = oldSize || "", coffee: CoffeeType | string | string[] = oldCoffee || "";
            ...
            requestedCoffees.push({ count, size: (size as CoffeeSizeType), coffee: (coffee as CoffeeType), price });
            //reset();
            if (empty) {
                empty = false;
                i = 0;
    ...
    requestedCoffees.map(i => counts.indexOf(i.count as string) + 1).forEach(i => {
...
export const prepareResponse = (response: IResponse, data: IConnectionData, request: IRequest) => {
    const { missedSize, name, nameRequested, oldCoffee, oldCount, oldRequestedCoffees, oldSize } = data;
    ...
    if (!noSize.length && !noCoffee.length && name!.length) {
        ...
        oldRequestedCoffees!.forEach((coffee, index, length) => {
            const newCoffeeOrder: ICoffeeOrder = {
                coffee: coffee.coffee.split(" ").map(i => capitalize(i)).join(" ") as CoffeeType,
                size: coffee.size,
                count: coffee.count ? counts.indexOf(coffee.count as CoffeeCountType) > -1 ? counts.indexOf(coffee.count as CoffeeCountType) + 1 : parseInt(coffee.count as string) : 1,
            }
